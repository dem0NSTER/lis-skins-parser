...
def main(lis_skins_base_url, fast_profit=False):
    ...
    while True:
        ...
        if not is_empty_json(f'{PATH_TO_PROJECT}/json/lis_skins.json'):
        ...
        if not is_empty_json(f'{PATH_TO_PROJECT}/json/cant_find.json'):
            cant_find_elements(fast_profit=fast_profit)

        if is_empty_json(f'{PATH_TO_PROJECT}/json/lis_skins.json') and is_empty_json(f'{PATH_TO_PROJECT}/json/cant_find.json'):
...
if __name__ == '__main__':
    ...
    fast_url = 'https://lis-skins.ru/market/csgo/?sort_by=hot&type_id=46%2C48%2C49%2C47%2C50%2C51&price_from=10&price_to=15&hold=-1'
